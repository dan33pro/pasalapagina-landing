  ...
  return (
    <main className="grid grid-cols-2 w-screen h-main">
      <aside className="flex items-center justify-center bg-center bg-no-repeat bg-cover" style={{backgroundImage: `url(${backgroundImage.src})`}}>
        {(state.isPaymentMethod || state.isPMCARD || state.isPMPSE) && <Discount />}
        {!(state.isPaymentMethod || state.isPMCARD || state.isPMPSE) && <Image className="w-[40vw] h-auto" src={personasImage} alt="Imagen Promocional - Tu edad, tu descuento" />}
      </aside>
      <aside>
        ...
        {(state.isPhoneConfirmation || state.isEmailConfirmation) && <SecurityConfirmation />}
