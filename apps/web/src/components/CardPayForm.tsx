...
const CardPayForm = () => {
  ...
  useEffect(() => {
    if (
        ...
        !isNaN(cuotas) && cuotas != "" &&
        expiracion.length > 0 &&
        !isNaN(cvv) && cvv != "" && parseInt(cvv) > 100 && parseInt(cvv) < 1000 &&
        !isNaN(cc) && cc != "" &&
        !isNaN(phone) && phone != "" &&
        country.length > 0 &&
        department.length > 0 &&
  ...
  }, [titular, numero, cuotas, expiracion, cvv, cc, phone, country, department, city, address]);

  return (
    <form className="max-w-[414px] px-4 grid gap-y-4 h-min" onSubmit={handlerSubmit}>
          ...
          <div className="text-xs text-center font-light">
              Al hacer clic en "PAGAR" estás aceptando los<br/>
              <span className="ml-2 text-[var(--light-blue)] underline"> Términos y Condiciones </span>
              de la Promoción.
          </div>
          <button
            type="submit"
